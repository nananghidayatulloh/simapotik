Public Class frmJenisGolObat
    Dim DtDataview As New DataView
    Dim objDataTable As New DataTable
    Dim bMgr As BindingManagerBase
    Dim JenisBindSource As New BindingSource

    Private Sub frmJenisGolObat_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles MyBase.KeyDown
    ...
    Public Sub ListViewDesign()
        ...
        ListView1.Columns.Add("Jenis Obat", 200, HorizontalAlignment.Center)
    End Sub

    Public Sub FillListview()
        Try
            Dim DataControl As New AccessData.DataControl
            Dim myData As DataSet = DataControl.GetDataSet("Select * from tbl_jenis_obat")
            ...
            For i = 0 To (objDataTable.Rows.Count - 1)
                With objDataTable.Rows(i)
                    ...
                    lSingleItem = ListView1.Items.Add(.Item("id_jenis_obat").ToString)
                    lSingleItem.SubItems.Add(.Item("nama_jenis").ToString)
                End With
            Next i
    ...
    Private Sub frmJenisGolObat_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ListViewDesign()
        FillListview()
    End Sub
    ...
    Private Sub txtSearch_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtSearch.TextChanged
        Try
            Dim DataControl As New AccessData.DataControl
            Dim myData As DataSet = DataControl.GetDataSet("Select * from tbl_jenis_obat where nama_jenis like '" & txtSearch.Text & "%'")
            objDataTable = myData.Tables("data")
            DtDataview.Table = objDataTable
            ListView1.Items.Clear()
            For i = 0 To (objDataTable.Rows.Count - 1)
    ...
    Private Sub ListView1_DoubleClick(ByVal sender As Object, ByVal e As System.EventArgs) Handles ListView1.DoubleClick
        ...
        frmMstJenisGolObat.Data = CInt(ListView1.Items(x).SubItems(0).Text)
        frmMstJenisGolObat.Opt = 2
        frmMstJenisGolObat.ShowDialog()
    End Sub
    ...
    Private Sub HapusToolStripMenuItem_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles HapusToolStripMenuItem.Click
        ...
        If MsgBox("Anda yakin ingin menghapus data : " & ListView1.Items(x).SubItems(1).Text & "?", MsgBoxStyle.Information Or MsgBoxStyle.YesNo, "Konfirmasi") = MsgBoxResult.Yes Then
            Try
                ...
                Dim AccessJenisGolObat As New AccessData.AccessJenisGolObat
                JenisGolonganObat.IdJenis = CInt(ListView1.Items(x).SubItems(0).Text)
                AccessJenisGolObat.JenisGolonganObatDelete(JenisGolonganObat.IdJenis)
            Catch ex As Exception
                MsgBox(ex.Message)
            End Try
        End If
